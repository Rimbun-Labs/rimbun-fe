...
const BusinessPlansPage: React.FC<{ customerIdOverride?: string }> = ({
  ...
  return (
    <BusinessWorkspaceShell
      ...
      description="Future events that should influence the cash outlook."
    ...
    >
      {(ws) => (
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Add plan event</CardTitle>
              <CardDescription>Hire, purchase, contract, or cost/revenue change.</CardDescription>
            </CardHeader>
          ...
          <div className="space-y-3">
            {ws.planEvents.length === 0 ? (
              <p className="text-sm text-muted-foreground">No plan events yet.</p>
            ) : (
              ws.planEvents.map((p) => (
                <Card key={p.id}>
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription>
                      ...
                      {p.amount != null ? ` · ${p.amount} ${p.currency ?? ""}` : ""} · source{" "}
                      {p.source}
                    </CardDescription>
                  </CardHeader>
                </Card>
