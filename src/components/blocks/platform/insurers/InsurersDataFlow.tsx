...
import { insurersDataSources, insurersOutputs } from "./content";
    ...
    <div>
      <div className="mx-auto max-w-[720px] text-center">
        <h2 className="text-[24px] font-semibold tracking-tight text-foreground md:text-[28px]">
          Deeper context for better risk and lending decisions
        </h2>
        <p className="mt-2 text-[16px] text-[#6b7280] dark:text-muted-foreground">
          Combine internal data with external signals to see the full picture.
        </p>
      </div>

      <div className="mt-8">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 lg:gap-3">
          {insurersDataSources.map((source) => {
            ...
            <div className="flex min-w-0 items-start gap-2.5">
              ...
              <div>
                ...
                <p className="mt-1 max-w-[360px] text-[13px] leading-relaxed text-[#6b7280] dark:text-muted-foreground">
                  We connect and analyze your data to assess risk, detect
                  emerging issues, and identify opportunities.
                </p>
              </div>
            </div>
            <div className="grid shrink-0 grid-cols-2 gap-1.5 sm:grid-cols-3">
              {insurersOutputs.map((tag) => (
                <span
                  key={tag}
